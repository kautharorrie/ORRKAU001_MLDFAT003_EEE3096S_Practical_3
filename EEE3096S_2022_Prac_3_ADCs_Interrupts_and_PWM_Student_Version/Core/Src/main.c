...
//	HAL_Delay (500); // wait for 500ms
	...
	HAL_StatusTypeDef status = HAL_ADC_Start(&hadc); //start the ADC and check the status
		if (status != HAL_OK){
			return 0;
		}
		status =  HAL_ADC_PollForConversion(&hadc, 1000); //poll the ADC
		if (status == HAL_OK){
			uint32_t val =  HAL_ADC_GetValue(&hadc);
			HAL_ADC_Stop(&hadc); // stop adc
			return val; //return the current value of the ADC
		}
		return 0; //return zero if it hasn't returned anything yet
...
uint32_t ADCtoCRR(uint32_t adc_val){
	...
	float dc = adc_val / 4096.0;
	float crr = dc*47999;
	__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_4, (int) crr);
	return crr;
}
